...
C<=item text> B<text>를 이름으로 하는 요소를 생성한다. 첫 번째 요소의 B<text>자리가 B<* >로 시작하면 순서가 없는 리스트를 구성하게 된다.
...
각각 I<italic>, B<bold>, C<code>, L<link|http://www.blog_villain.com>, C<< B<E<lt>> >> (=EE<lt>escapeE<gt>)을 나타낸다.
